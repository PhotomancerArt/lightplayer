//! The per-connection **learned table** (format 2), HPACK-style.
//!
//! The format's key and value codes index `seed ++ learned`: the first
//! `seed.len()` codes are the injected [`Dictionary`](crate::Dictionary) (empty
//! on LightPlayer's wire), the rest this table. An inline key, or an inline
//! string on its **second** inline sighting, *defines* the next learned entry
//! as a side effect of being sent (HPACK's "literal with incremental
//! indexing"), so a name costs its inline bytes once per connection and a code
//! after that. No tag exists for it.
//!
//! Both ends must learn identically, so what qualifies and the capacities are
//! **format**, fixed by [`PACK_FORMAT_VERSION`](crate::PACK_FORMAT_VERSION):
//! the `LEARN_*` constants here. When a side is full, learning stops (no
//! eviction). Values are learned on their second sighting because most
//! one-off strings (file paths, node names) would otherwise fill the table:
//! 57 % of the distinct value strings of a recorded Studio session were seen
//! exactly once (plan `lp2025/2026-09-25-0006-learned-wire-dictionary`).
//!
//! **Commit/rollback.** A frame's definitions are tentative. The encoder side
//! takes a [`LearnMark`] before a frame and [`LearnStore::truncate`]s back to
//! it when the frame is not sent (a JSON fallback, or a write abandoned); the
//! decoder side truncates when a frame does not decode. Truncation rebuilds
//! the hash indexes (a rare path).
//!
//! **Frame header.** A learned frame starts with the table's epoch (one byte)
//! and a 16-bit fold of a rolling hash over everything the table learned and
//! every first sighting it remembered, in order (two bytes, little-endian).
//! A reader whose own table disagrees must not decode the frame: see
//! [`read_header`]. An entry count is not enough: after a torn frame the two
//! sides can learn *different* entries and still hold the same number.

use crate::pack_dictionary::fnv1a;

/// Text bytes a table holds, shared by keys and values.
pub const LEARN_TEXT_CAP: usize = 3072;
/// Keys a table holds.
pub const LEARN_KEYS_CAP: usize = 288;
/// Values a table holds.
pub const LEARN_VALUES_CAP: usize = 128;
/// First sightings a table remembers (the second-sighting rule's log).
pub const LEARN_SEEN_CAP: usize = 512;
/// Longest key the table learns.
pub const LEARN_KEY_MAX_LEN: usize = 48;
/// Shortest value string the table learns.
pub const LEARN_VALUE_MIN_LEN: usize = 2;
/// Longest value string the table learns (the inline-length tag's reach).
pub const LEARN_VALUE_MAX_LEN: usize = 31;
/// Bytes a learned frame's header takes: epoch, then the folded state.
pub const LEARN_HEADER_LEN: usize = 3;

/// Hash slots per side: powers of two, at least one empty slot per side.
const KEY_HASH_SLOTS: usize = 512;
const VALUE_HASH_SLOTS: usize = 256;

/// An empty hash slot. Slots hold entry index + 1, so a new table is all
/// zeroes: a static one lands in `.bss`, not flash-backed `.data`, and a heap
/// one can be allocated zeroed.
const SLOT_EMPTY: u16 = 0;

/// The state hash of an empty table (zero, for the same reason).
pub const STATE_EMPTY: u32 = 0;

/// A table position: what [`LearnStore::truncate`] returns to.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub struct LearnMark {
    /// Keys held.
    pub keys: usize,
    /// Values held.
    pub values: usize,
    /// Text bytes held.
    pub text: usize,
    /// First sightings remembered.
    pub seen: usize,
    /// The table's rolling state hash.
    pub state: u32,
}

/// What the encoder and decoder need of a learned table. Object-safe, so the
/// codec's types do not carry the table's.
pub trait LearnStore {
    /// The epoch frames are coded in.
    fn epoch(&self) -> u8;
    /// Where the table stands now.
    fn mark(&self) -> LearnMark;
    /// Drop everything learned after `mark`.
    fn truncate(&mut self, mark: LearnMark);
    /// Empty the table and start `epoch`.
    fn reset(&mut self, epoch: u8);
    /// The index of learned key `text`.
    fn find_key(&self, text: &[u8]) -> Option<usize>;
    /// The index of learned value `text`.
    fn find_value(&self, text: &[u8]) -> Option<usize>;
    /// Learned key `i`.
    fn key(&self, i: usize) -> Option<&[u8]>;
    /// Learned value `i`.
    fn value(&self, i: usize) -> Option<&[u8]>;
    /// An inline key went by: learn it if it qualifies and there is room.
    fn learn_key(&mut self, text: &[u8]);
    /// An inline string went by: learn it on its second sighting, if it
    /// qualifies and there is room.
    fn learn_value(&mut self, text: &[u8]);
}

/// A learned table at the format's capacities: about 6.9 KB, all zero when
/// new. The board holds one per packed link, the host one per link it reads.
#[derive(Clone)]
pub struct LearnedTable {
    text: [u8; LEARN_TEXT_CAP],
    text_len: usize,
    keys: Side<LEARN_KEYS_CAP, KEY_HASH_SLOTS>,
    values: Side<LEARN_VALUES_CAP, VALUE_HASH_SLOTS>,
    epoch: u8,
    /// First sightings: a 16-bit fold of each string's FNV-1a.
    seen: [u16; LEARN_SEEN_CAP],
    seen_count: usize,
    /// A rolling hash of everything learned and every first sighting
    /// remembered, in order: what the frame header carries.
    state: u32,
}

impl LearnedTable {
    /// An empty table in epoch 0.
    pub const NEW: Self = Self {
        text: [0; LEARN_TEXT_CAP],
        text_len: 0,
        keys: Side::NEW,
        values: Side::NEW,
        epoch: 0,
        seen: [0; LEARN_SEEN_CAP],
        seen_count: 0,
        state: STATE_EMPTY,
    };

    /// An empty table in epoch 0 on the heap, allocated zeroed so the ~6.9 KB
    /// never passes through the stack.
    #[cfg(feature = "alloc")]
    pub fn boxed() -> alloc::boxed::Box<Self> {
        let zeroed = alloc::boxed::Box::<Self>::new_zeroed();
        // SAFETY: every field is an integer or an array of integers, and the
        // all-zero bit pattern is `Self::NEW` (`SLOT_EMPTY` and `STATE_EMPTY`
        // are zero, and every other field of `NEW` is a zero integer).
        unsafe { zeroed.assume_init() }
    }

    /// Whether `s` was seen inline before; remembers it if not (and there is
    /// room). A linear scan: it runs only for strings not yet learned.
    fn second_sighting(&mut self, s: &[u8]) -> bool {
        let f = fnv1a(s);
        let h = (f ^ (f >> 16)) as u16;
        if self.seen[..self.seen_count].contains(&h) {
            return true;
        }
        if self.seen_count < LEARN_SEEN_CAP {
            self.seen[self.seen_count] = h;
            self.seen_count += 1;
            self.state = mix(self.state, b's', &h.to_le_bytes());
        }
        false
    }

    fn push_text(&mut self, s: &[u8]) -> Option<u16> {
        let at = self.text_len;
        let end = at + s.len();
        if end > LEARN_TEXT_CAP {
            return None;
        }
        self.text[at..end].copy_from_slice(s);
        self.text_len = end;
        Some(at as u16)
    }
}

impl LearnStore for LearnedTable {
    fn epoch(&self) -> u8 {
        self.epoch
    }

    fn mark(&self) -> LearnMark {
        LearnMark {
            keys: self.keys.count,
            values: self.values.count,
            text: self.text_len,
            seen: self.seen_count,
            state: self.state,
        }
    }

    fn truncate(&mut self, mark: LearnMark) {
        if mark == self.mark() {
            return;
        }
        self.seen_count = mark.seen.min(self.seen_count);
        self.state = mark.state;
        self.text_len = mark.text.min(self.text_len);
        let text = &self.text[..self.text_len];
        self.keys.rebuild(text, mark.keys);
        self.values.rebuild(text, mark.values);
    }

    fn reset(&mut self, epoch: u8) {
        self.truncate(LearnMark::default());
        self.epoch = epoch;
    }

    fn find_key(&self, text: &[u8]) -> Option<usize> {
        self.keys.find(&self.text, text)
    }

    fn find_value(&self, text: &[u8]) -> Option<usize> {
        self.values.find(&self.text, text)
    }

    fn key(&self, i: usize) -> Option<&[u8]> {
        self.keys.get(&self.text, i)
    }

    fn value(&self, i: usize) -> Option<&[u8]> {
        self.values.get(&self.text, i)
    }

    fn learn_key(&mut self, s: &[u8]) {
        if s.is_empty() || s.len() > LEARN_KEY_MAX_LEN || self.keys.count >= LEARN_KEYS_CAP {
            return;
        }
        if let Some(at) = self.push_text(s) {
            self.keys.push(&self.text, at, s.len());
            self.state = mix(self.state, b'k', s);
        }
    }

    fn learn_value(&mut self, s: &[u8]) {
        if !(LEARN_VALUE_MIN_LEN..=LEARN_VALUE_MAX_LEN).contains(&s.len())
            || self.values.count >= LEARN_VALUES_CAP
            || !self.second_sighting(s)
        {
            return;
        }
        if let Some(at) = self.push_text(s) {
            self.values.push(&self.text, at, s.len());
            self.state = mix(self.state, b'v', s);
        }
    }
}

/// Write a learned frame's header (epoch, then the table's state hash folded
/// to 16 bits, little-endian) into `out`. Returns [`LEARN_HEADER_LEN`], or
/// `None` when `out` is too small.
pub fn write_header(out: &mut [u8], store: &dyn LearnStore) -> Option<usize> {
    let s = fold(store.mark().state);
    out.get_mut(..LEARN_HEADER_LEN)?
        .copy_from_slice(&[store.epoch(), s as u8, (s >> 8) as u8]);
    Some(LEARN_HEADER_LEN)
}

/// Why a learned frame's header does not match the reader's table.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HeaderMismatch {
    /// The frame is shorter than a header.
    Truncated,
    /// The board is in another epoch and this frame is not its reset: the
    /// reader missed the reset frame.
    Epoch {
        /// The frame's epoch.
        frame: u8,
        /// The reader's.
        reader: u8,
    },
    /// Same epoch, different table: a frame that changed the table was lost
    /// or torn without the board knowing, and the two sides have diverged.
    State {
        /// The frame's folded state.
        frame: u16,
        /// The reader's.
        reader: u16,
    },
}

/// Read a learned frame's header and check it against `store`. On a match
/// returns the header length (the value starts there).
///
/// A frame of a **new** epoch whose state is the empty table's is the board's
/// announced reset: the reader resets `store` to that epoch and decodes it.
pub fn read_header(frame: &[u8], store: &mut dyn LearnStore) -> Result<usize, HeaderMismatch> {
    let h = frame
        .get(..LEARN_HEADER_LEN)
        .ok_or(HeaderMismatch::Truncated)?;
    let (epoch, state) = (h[0], u16::from_le_bytes([h[1], h[2]]));
    if epoch != store.epoch() {
        if state == fold(STATE_EMPTY) {
            store.reset(epoch);
            return Ok(LEARN_HEADER_LEN);
        }
        return Err(HeaderMismatch::Epoch {
            frame: epoch,
            reader: store.epoch(),
        });
    }
    let have = fold(store.mark().state);
    if state != have {
        return Err(HeaderMismatch::State {
            frame: state,
            reader: have,
        });
    }
    Ok(LEARN_HEADER_LEN)
}

/// One side (keys or values) of a [`LearnedTable`].
#[derive(Clone)]
struct Side<const N: usize, const H: usize> {
    start: [u16; N],
    len: [u8; N],
    count: usize,
    hash: [u16; H],
}

impl<const N: usize, const H: usize> Side<N, H> {
    const NEW: Self = Self {
        start: [0; N],
        len: [0; N],
        count: 0,
        hash: [SLOT_EMPTY; H],
    };

    fn get<'t>(&self, text: &'t [u8], i: usize) -> Option<&'t [u8]> {
        if i >= self.count {
            return None;
        }
        let a = usize::from(self.start[i]);
        text.get(a..a + usize::from(self.len[i]))
    }

    fn find(&self, text: &[u8], needle: &[u8]) -> Option<usize> {
        if self.count == 0 {
            return None;
        }
        let mask = H - 1;
        let mut slot = fnv1a(needle) as usize & mask;
        for _ in 0..H {
            let v = self.hash[slot];
            if v == SLOT_EMPTY {
                return None;
            }
            let i = usize::from(v - 1);
            if self.get(text, i) == Some(needle) {
                return Some(i);
            }
            slot = (slot + 1) & mask;
        }
        None
    }

    fn push(&mut self, text: &[u8], at: u16, len: usize) {
        let i = self.count;
        self.start[i] = at;
        self.len[i] = len as u8;
        self.count += 1;
        self.index(text, i);
    }

    fn index(&mut self, text: &[u8], i: usize) {
        let mask = H - 1;
        let e = self.get(text, i).unwrap_or(&[]);
        let mut slot = fnv1a(e) as usize & mask;
        while self.hash[slot] != SLOT_EMPTY {
            slot = (slot + 1) & mask;
        }
        self.hash[slot] = i as u16 + 1;
    }

    fn rebuild(&mut self, text: &[u8], count: usize) {
        self.count = count.min(self.count);
        self.hash = [SLOT_EMPTY; H];
        for i in 0..self.count {
            self.index(text, i);
        }
    }
}

fn mix(h: u32, kind: u8, bytes: &[u8]) -> u32 {
    let mut h = (h ^ u32::from(kind)).wrapping_mul(0x0100_0193);
    for &b in bytes {
        h = (h ^ u32::from(b)).wrapping_mul(0x0100_0193);
    }
    (h ^ bytes.len() as u32).wrapping_mul(0x0100_0193)
}

fn fold(state: u32) -> u16 {
    (state ^ (state >> 16)) as u16
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn keys_learn_on_first_sighting_until_full() {
        let mut t = LearnedTable::NEW;
        t.learn_key(b"");
        t.learn_key(&[b'k'; LEARN_KEY_MAX_LEN + 1]);
        assert_eq!(t.mark().keys, 0);
        for i in 0..LEARN_KEYS_CAP + 5 {
            t.learn_key(format_key(i).as_bytes());
        }
        assert_eq!(t.mark().keys, LEARN_KEYS_CAP);
        assert_eq!(t.find_key(format_key(7).as_bytes()), Some(7));
        assert_eq!(t.find_key(format_key(LEARN_KEYS_CAP).as_bytes()), None);
        assert_eq!(t.key(3), Some(format_key(3).as_bytes()));
        assert_eq!(t.key(LEARN_KEYS_CAP), None);
    }

    #[test]
    fn values_learn_on_second_sighting() {
        let mut t = LearnedTable::NEW;
        t.learn_value(b"x"); // too short
        t.learn_value(b"x");
        t.learn_value(&[b'v'; LEARN_VALUE_MAX_LEN + 1]); // too long
        t.learn_value(&[b'v'; LEARN_VALUE_MAX_LEN + 1]);
        assert_eq!(t.mark().seen, 0);
        t.learn_value(b"rgb");
        assert_eq!(t.find_value(b"rgb"), None);
        assert_eq!(t.mark().seen, 1);
        t.learn_value(b"rgb");
        assert_eq!(t.find_value(b"rgb"), Some(0));
        assert_eq!(t.mark().values, 1);
    }

    #[test]
    fn values_stop_at_capacity() {
        let mut t = LearnedTable::NEW;
        for i in 0..LEARN_VALUES_CAP + 3 {
            let v = format_key(i);
            t.learn_value(v.as_bytes());
            t.learn_value(v.as_bytes());
        }
        assert_eq!(t.mark().values, LEARN_VALUES_CAP);
    }

    #[test]
    fn text_capacity_stops_both_sides() {
        let mut t = LearnedTable::NEW;
        let long = [b'a'; LEARN_KEY_MAX_LEN];
        let mut n = 0;
        while t.mark().text + LEARN_KEY_MAX_LEN <= LEARN_TEXT_CAP {
            let mut k = long;
            k[..2].copy_from_slice(&(n as u16).to_le_bytes());
            t.learn_key(&k);
            n += 1;
        }
        let before = t.mark();
        t.learn_key(&[b'z'; LEARN_KEY_MAX_LEN]);
        assert_eq!(t.mark(), before);
    }

    #[test]
    fn truncate_forgets_and_reindexes() {
        let mut t = LearnedTable::NEW;
        t.learn_key(b"a1");
        t.learn_value(b"v3");
        let m = t.mark();
        t.learn_key(b"b2");
        t.learn_value(b"v3"); // learned now
        t.learn_value(b"v4"); // a first sighting
        t.truncate(m);
        assert_eq!(t.mark(), m);
        assert_eq!(t.find_key(b"b2"), None);
        assert_eq!(t.find_value(b"v3"), None);
        assert_eq!(t.find_key(b"a1"), Some(0));
        t.learn_value(b"v4"); // its first sighting was rolled back too
        assert_eq!(t.find_value(b"v4"), None);
        t.learn_value(b"v3"); // its first sighting was kept
        assert_eq!(t.find_value(b"v3"), Some(0));
    }

    #[test]
    fn reset_empties_and_changes_epoch() {
        let mut t = LearnedTable::NEW;
        t.learn_key(b"k");
        t.reset(9);
        assert_eq!(t.epoch(), 9);
        assert_eq!(t.mark(), LearnMark::default());
        assert_eq!(t.find_key(b"k"), None);
    }

    #[test]
    fn header_detects_divergence_and_accepts_announced_reset() {
        let mut board = LearnedTable::NEW;
        let mut host = LearnedTable::NEW;
        let mut h = [0u8; 8];
        let n = write_header(&mut h, &board).unwrap();
        assert_eq!(read_header(&h[..n], &mut host), Ok(LEARN_HEADER_LEN));

        board.learn_key(b"k1");
        let n = write_header(&mut h, &board).unwrap();
        assert!(matches!(
            read_header(&h[..n], &mut host),
            Err(HeaderMismatch::State { .. })
        ));

        // A new epoch that is not empty is not a reset.
        board.reset(1);
        board.learn_key(b"k2");
        let n = write_header(&mut h, &board).unwrap();
        assert_eq!(
            read_header(&h[..n], &mut host),
            Err(HeaderMismatch::Epoch {
                frame: 1,
                reader: 0
            })
        );

        board.reset(2);
        let n = write_header(&mut h, &board).unwrap();
        assert_eq!(read_header(&h[..n], &mut host), Ok(n));
        assert_eq!(host.epoch(), 2);
        assert_eq!(read_header(&h[..2], &mut host), Err(HeaderMismatch::Truncated));
        assert_eq!(write_header(&mut h[..2], &board), None);
    }

    #[test]
    fn same_count_different_entries_is_a_mismatch() {
        // The case a count-only header misses: equal counts, different names.
        let mut board = LearnedTable::NEW;
        let mut host = LearnedTable::NEW;
        board.learn_key(b"alpha");
        host.learn_key(b"omega");
        assert_eq!(board.mark().keys, host.mark().keys);
        let mut h = [0u8; 3];
        write_header(&mut h, &board).unwrap();
        assert!(matches!(
            read_header(&h, &mut host),
            Err(HeaderMismatch::State { .. })
        ));
    }

    #[cfg(feature = "alloc")]
    #[test]
    fn boxed_is_new() {
        let mut b = LearnedTable::boxed();
        let mut n = LearnedTable::NEW;
        assert_eq!(b.mark(), n.mark());
        assert_eq!(b.epoch(), 0);
        for t in [&mut *b as &mut dyn LearnStore, &mut n] {
            t.learn_key(b"kind");
            t.learn_value(b"rgb");
            t.learn_value(b"rgb");
        }
        assert_eq!(b.mark(), n.mark());
        assert_eq!(b.find_key(b"kind"), Some(0));
        assert_eq!(b.find_value(b"rgb"), Some(0));
    }

    #[test]
    fn a_table_is_about_seven_kilobytes() {
        assert!(core::mem::size_of::<LearnedTable>() < 7 * 1024);
    }

    fn format_key(i: usize) -> alloc_free::Key {
        alloc_free::Key::new(i)
    }

    /// A tiny no-alloc formatter, so the tests run without `alloc`.
    mod alloc_free {
        pub struct Key([u8; 8]);
        impl Key {
            pub fn new(i: usize) -> Self {
                let mut b = *b"key00000";
                let mut n = i;
                for j in (3..8).rev() {
                    b[j] = b'0' + (n % 10) as u8;
                    n /= 10;
                }
                Key(b)
            }
            pub fn as_bytes(&self) -> &[u8] {
                &self.0
            }
        }
    }
}
