...
| `mini-dome` | `speed`, `bands`, `warmth` (per submodule) | the patching archetype at install scale: a 5-way repeated 2V geodesic dome (each sector = three suspended triangle panels wired as one gapped path; map2d format 3 stable ids) and three always-lit chevron doors scattered across TWO named outputs ("1", "Box 2") with shared ports — many-to-many. The `.patch.json` files are format-2 path-identity rows (`/sector/2`) carrying the as-built permutation, one sector reversed, one rotated by a panel, one door turned by its authored stride. See [patching the dome](../docs/user-guide/patching-the-dome.md) |
