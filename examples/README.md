...
`palette-waves`, `fire2012`, `plasma-duo`, `zook-dome`, `mini-dome`,
`peach-1d`, `peach-2d`.
...
| `mini-dome` | `speed`, `bands`, `warmth` (per submodule) | the patching archetype at install scale: a 5-way repeated 2V geodesic dome (each sector = eight suspended triangle panels, one per 2V face, wired as one gapped path; geometry generated by `cargo run -p lpt-geodome`; map2d format 3 stable ids) and three always-lit chevron doors scattered across TWO named outputs ("1", "Box 2") with shared ports — many-to-many. The `.patch.json` files are format-2 path-identity rows (`/sector/2`) carrying the as-built permutation, one sector reversed, one rotated, one door turned by its authored stride. See [patching the dome](../docs/user-guide/patching-the-dome.md) |
