...
const GOLDEN: &[Expectation] = &[
    ...
    // The small-dome: TWO producers scattered across TWO named outputs
    ...
    // to match the real dome, and again 2026-08-28 when the example (then
    // "mini-dome", a 177-lamp miniature) was rebuilt at FULL scale — 50
    // panels x 119 lamps + a 360-lamp door across 26 ports, generated by
    // `cargo run -p lpt-geodome`. Lamp counts, positions, and patch files
    // all moved; the placements are pinned by `small_dome_scatter`.
    ...
    Expectation {
        ...
        outputs: &[
            (
                "/small_dome.show/out_a.output",
                &[
                    (20010, 0x5faba9f88665ce0d, [0, 0, 0, 0, 0, 0]),
                    (20010, 0x2a98e876479251b2, [185, 6, 139, 23, 185, 43]),
                    (20010, 0x13f23661db633609, [192, 6, 164, 23, 232, 43]),
                    (20010, 0x7fac526c49ce1f2a, [201, 6, 192, 23, 28, 44]),
                ],
            ),
            (
                "/small_dome.show/out_b.output",
                &[
                    (0, 0xcbf29ce484222325, [0, 0, 0, 0, 0, 0]),
                    (17850, 0x718c452b09c1e9d5, [244, 212, 159, 123, 141, 45]),
                    (17850, 0xd71cefee2607246e, [121, 211, 190, 122, 49, 45]),
                    (17850, 0x5718db32ea85a5dc, [237, 209, 211, 121, 211, 44]),
                ],
            ),
        ],
    },
