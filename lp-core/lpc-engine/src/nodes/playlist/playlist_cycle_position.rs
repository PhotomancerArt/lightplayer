...
//!
//! The cycle's **stops** are the authored entries in key order that may play
...
//! A2).
//!
//! The **anchor** `(entry, time)` is where the cycle last started counting:
//! a pick, a trigger or next/prev sets it to `(picked, now)`, so the cycle
...
//! ```
//!
//! `t` is the playlist's consumed `time` — the clock, so the cycle follows
//! its speed and pause exactly as palette cycling does. At `k = 0` the cycle
//! is on the anchor entry itself, even when that entry is no longer a stop
//! (skipped while playing): it plays out its step, then the cycle moves on.

use super::PlaylistRuntimeEntry;

/// The cycle's anchor: the entry it started counting from, and when.
#[derive(Clone, Copy, Debug, PartialEq)]
pub(super) struct PlaylistCycleAnchor {
    pub entry: u32,
    pub time: f32,
}

impl PlaylistCycleAnchor {
    ...
    /// The same step phase, counted from `entry`: the anchor the cycle moves
...
mod tests {
    ...
    fn two_second_steps_walk_the_stops_in_order_and_wrap() {
        ...
        let anchor = PlaylistCycleAnchor::new(1, 0.0);

        let walk: Vec<u32> = (0..=10)
            .map(|t| cycle_entry_at(&entries, anchor, 2.0, t as f32).unwrap())
            .collect();
    ...
    fn skipped_and_failed_entries_are_passed_over() {
        ...
        let walk: Vec<u32> = (0..5)
            .map(|step| cycle_entry_at(&entries, anchor, 2.0, step as f32 * 2.0).unwrap())
            .collect();
    ...
    fn an_anchor_that_is_not_a_stop_plays_out_its_step_then_moves_on() {
        ...
        entries[1].reason = PlaylistEntryReason::Disabled;
        let anchor = PlaylistCycleAnchor::new(2, 10.0);

        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 11.0), Some(2));
        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 12.0), Some(3));
        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 14.0), Some(1));
    }

    #[test]
    fn a_clock_scrubbed_back_walks_backwards() {
        let entries = entries(&[1, 2, 3]);
        let anchor = PlaylistCycleAnchor::new(1, 10.0);

        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 9.0), Some(3));
        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 7.0), Some(2));
    }

    #[test]
    fn nothing_to_cycle_holds() {
        ...
        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 1.0), Some(1), "k = 0");
        assert_eq!(cycle_entry_at(&entries, anchor, 2.0, 2.0), None);
    }
