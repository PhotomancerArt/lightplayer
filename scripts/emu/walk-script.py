...
def main() -> int:
    ...
    lines = [
        ...
        f"#   {args.transcript}",
        "# Every request waits for the answer to the one before it, so the",
        "# whole walk is a function of guest time. Do not hand-edit: re-run",
        "# the capture and regenerate.",
        "",
    ]
    ...
    for chunk in chunks:
        for frame in frames(chunk):
            fid = frame_id(frame)
            lines.append(f'after "{escape(needle.encode())}" "{escape(frame)}"')
            count += 1
            if fid is None:
                print(
