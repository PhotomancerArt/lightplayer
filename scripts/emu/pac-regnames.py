...
ESP32S3_TARGETS = [
    ...
    _s3("uart0", "UART0"),
    _s3("sha", "SHA"),
    # M6 P04: the mask ROM's cache setup reads it before anything the
    # application's own literal census could see — P03's first strict stop
    # was `Cache_Occupy_ICache_MEMORY+0xc` reading `+0x04`
    # (`cache_dataarray_connect_1`) at cycle 36, on
    # `rom_config_instruction_cache_mode`'s path.
    _s3("sensitive", "SENSITIVE"),
]
...
class Chip:
    ...
    # Which PAC file holds that enum. svd2rust puts it in `src/interrupt.rs`
    # for the C6; the S3's PAC keeps it in `src/lib.rs` (M6 P04).
    sources_file: str = "src/interrupt.rs"
...
CHIPS = {
    ...
    "esp32s3": Chip(
        ...
        # The S3 HAS an interrupt-source enum, and its PAC keeps it in
        # `src/lib.rs` rather than in a `src/interrupt.rs` as the C6 does
        # (`esp32s3-0.35.2/src/lib.rs:325`, 94 named variants numbered
        # 0..=98 with gaps). `sources_file` is the one-line generator change
        # M6 P01 left for the phase that first needed the table (P04's
        # matrix); the numbers are read out of the PAC like every other
        # table's, never transcribed.
        sources_out="lp-emu/esp/lp-emu-esp32s3/src/regs/interrupt_sources.rs",
        sources_file="src/lib.rs",
    ),
}
...
def collect_sources(pac: PacSource) -> list[tuple[int, str]]:
    text = pac.read(pac.chip.sources_file)
    if text is None:
        raise SystemExit(f"pac-regnames: {pac.chip.pac} has no {pac.chip.sources_file}")
...
def render_sources(chip: Chip, entries: list[tuple[int, str]], svd2rust: str) -> str:
    lines = [
        ...
        f"//   {chip.pac}/{chip.sources_file}  (crate {chip.pac} {chip.version},",
