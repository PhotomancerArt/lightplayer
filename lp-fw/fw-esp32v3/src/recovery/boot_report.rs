...
use super::esp32v3_recovery_backend::Esp32V3RecoveryBackend;
...
pub fn init_and_report() -> BootAssessment {
    ...
    let (recovery, assessment) =
        lp_recovery::Recovery::init(Esp32V3RecoveryBackend::take(), reset_cause);
...
/// the same wording discipline as the panic path: a target with a walker must
/// not report an *empty* stack. See `panic_path::print_frames`.
fn log_crash_pcs(count: usize, pcs: &[u32]) {
    ...
    esp_println::print!("[RECOVERY] decode: just decode-backtrace-esp32v3");
    for pc in pcs {
