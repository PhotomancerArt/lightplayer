//! The ESP32-C6 hardware-derived golden vector: a WS2812 frame as a Seeed XIAO
//! ESP32-C6 actually put it on the wire, captured by the P5 RMT loopback
//! self-test at 12.5 ns resolution (see `golden/ws2812_grb_esp32c6.txt` for
//! provenance).
//!
//! Same shape as `hardware_golden.rs`, which does this for the ESP32-S3, and
//! deliberately a separate file: the two vectors are independent evidence from
//! two RMT generations, and folding them into one parameterised test would let
//! a chip that stopped matching hide behind the other's pass.
//!
//! What this pins on the host forever:
//!
//! * the parser/classifier agrees with C6 silicon, not just with the mock;
//! * the encoder's nominal tick values are what that hardware measured;
//! * a re-derived golden (README: re-run the loopback, transcribe the
//!   `golden_*` lines) is validated by exactly this code;
//! * the C6 and S3 captures are still the same waveform — the cross-chip claim
//!   in both provenance headers is asserted, not just written down.

use lp_ws281x::{ChannelTiming, ColorOrder, PulseCodes, PulseItem};

const GOLDEN: &str = include_str!("golden/ws2812_grb_esp32c6.txt");

/// The ESP32-S3 capture of the same frame, for the cross-chip comparison.
...
#[test]
fn the_c6_golden_vector_decodes_to_the_transmitted_frame() {
    ...
    assert_eq!(
        decoded, wire,
        "ESP32-C6 hardware capture must decode to the sent frame"
    );
}

#[test]
fn the_c6_golden_vector_sits_within_tolerance_of_the_configuration() {
    let bits = golden_bits(GOLDEN);
    let timing = ChannelTiming::WS2812;
    let codes = PulseCodes::at_default_clock(&timing).unwrap();
...
/// Both provenance headers claim the C6 and S3 captures are byte-for-byte
/// equal. Assert it, so an edited vector cannot leave the claim standing.
///
/// Two RMT generations with different channel counts and different RAM offsets,
/// driven by the same encoder at 80 MHz, produced the same 48 pulse pairs —
/// including what each chip's *own* receiver measured them as. If a future
/// re-derivation breaks this, that is a real chip difference and belongs in the
/// notes, not in a loosened assertion.
#[test]
fn the_c6_and_s3_captures_are_the_same_waveform() {
    assert_eq!(
        golden_bits(GOLDEN),
        golden_bits(GOLDEN_S3),
        "ESP32-C6 and ESP32-S3 captures of the same frame must agree tick for tick"
    );
}
