//! The corpus and the tick loop. Every printed line and every JSON record
//! here is a SHAPE the host side parses, so none of them is free to move:
//! `lp_emu_validate::payload::COMPILE_TICK`'s regex reads the per-tick line
//! and the `shader-compile-stress` field table reads `case-summary` and
//! `total-summary`, for all three chips, with one parser.
...
/// The per-tick step budget. The C6's and the classic's, unchanged — a slice
/// is a slice on any chip, and changing it here would make the three `ticks`
/// counts incomparable, which is the one structural field this payload has.
...
/// The corpus. One case, the same one the other two chips record —
/// `projects/test/basic`, which is in the tree at a stable path and is what
/// every committed `shader-compile-stress` transcript is of.
...
/// `CCOUNT`, the LX7's own free-running cycle counter — this chip's module
/// rather than a second copy of the `rsr.ccount` asm.
fn read_cycles() -> u32 {
    crate::board::esp32s3::cycle_counter::read()
}

/// …and its conversion, which carries `constants::CPU_HZ` (240 MHz, the rate
/// the harness entrypoint's `CpuClock::max()` actually sets).
fn cycles_to_us(cycles: u64) -> u64 {
    crate::board::esp32s3::cycle_counter::cycles_to_us(cycles)
}
