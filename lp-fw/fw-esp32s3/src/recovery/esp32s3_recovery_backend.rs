...
//!
//! This is the S3's implementation of `lp_recovery::RecoveryBackend`. The
//! region format, its validation, and the torn-write discipline all live in
//! `lp-recovery` and are chip-agnostic; the only chip knowledge here is *where*
//! the bytes live (RTC fast RAM, which survives software and watchdog resets
//! but not power loss) and *how* to reset (the ROM software-reset entry).
...
/// The breadcrumb region. RTC fast RAM survives software and watchdog resets
/// (NOT power loss); `persistent` skips load-time initialization so the
/// previous run's bytes are still there on soft reset. On power-up the
/// contents are undefined — `lp_recovery` validates magic + CRCs and the reset
/// reason before trusting anything.
///
/// `RecoveryRegion` is budgeted at <= `lp_recovery::REGION_MAX_SIZE` (1 KiB),
/// which the S3's 8 KiB of RTC fast RAM absorbs comfortably.
...
pub struct Esp32S3RecoveryBackend {
    _private: (),
}

impl Esp32S3RecoveryBackend {
    ...
    pub fn take() -> Self {
        assert!(
            ...
            "Esp32S3RecoveryBackend::take() called twice"
        );
        Self { _private: () }
    }
}

impl RecoveryBackend for Esp32S3RecoveryBackend {
