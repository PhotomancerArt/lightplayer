...
//!
//! The RTC watchdog is the layer-2 backstop for hangs that no in-process hook
//! can catch (an infinite loop in JIT'd shader code hangs the whole device).
//! Stage 0 resets the system after [`WATCHDOG_TIMEOUT_MS`]; after the reboot
//! `lp-recovery` blames whatever the frame stack shows. On the abort tier this
//! backstop matters *more* than on the C6, not less: there is no `catch_unwind`
//! layer above it.
//!
...
/// Stage-0 reset timeout. Must exceed the worst legitimate stall of the server
/// loop (large shader compile), with margin.
...
/// How long the I/O task may go silent before the server loop stops feeding on
/// its behalf.
