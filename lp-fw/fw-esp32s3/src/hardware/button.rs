//! GPIO button driver, ported from `fw-esp32c6/src/hardware/button.rs`.
//!
//! Deliberately a per-chip copy — same pattern as the RMT/WS281x driver
//! (`crate::output::rmt::esp32s3_rmt_ws281x_driver`): `fw-esp32-common` must
//! stay free of esp-hal code (ADR 2026-07-29-per-chip-fw-toolchains), so the
//! chip-specific pieces (the `esp_hal::gpio` types, and the S3's own
//! `MAX_GPIO`) live here rather than there.
...
use lpc_hardware::{
    ...
    HwEndpointKind, HwEndpointSpec, HwError, HwRegistry,
};
...
fn gpio_number(address: &HwAddress) -> Result<u8, HardwareEndpointError> {
    ...
    if gpio > crate::board::esp32s3::constants::MAX_GPIO {
        return Err(HardwareEndpointError::UnsupportedConfig {
            reason: format!("ESP32-S3 GPIO {gpio} is outside the supported range"),
        });
    }
    Ok(gpio)
}
