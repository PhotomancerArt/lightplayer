...
fn the_ledger_triple_is_elicited_by_a_stop_all_on_the_wire() {
    ...
    // `[stack] heartbeat: high-water <used> B of 37224 B (<headroom> B headroom)`
    //
    ...
    // the gate's await state), 37,296 → 37,272 on
    // its own branch; merged into `lp-json-pack` (#805 into #795) the same
    // −24 B lands on 37,248 → 37,224, measured on the merged image's ELF.
    // The stop-all path is unchanged.
    let words: Vec<&str> = stack[0].split_whitespace().collect();
    let used: u32 = words[3].parse().expect("high-water bytes");
    assert_eq!(
        &words[4..7],
        &["B", "of", "37224"],
        "the S3's 37,224 B total: {}",
        stack[0]
    );
    ...
    assert_eq!(used + headroom, 37_224, "{}", stack[0]);
    assert!(used > 0 && used < 37_224, "{}", stack[0]);
    for line in &mem {
        assert!(
    ...
    assert!(
        ...
        "the S3 prints no `main stack` line; its 37,224 B total is in every `[stack]` \
