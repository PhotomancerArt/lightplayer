//! `SYSTIMER` at `0x6002_3000` — the 52-bit system timer, modelled; **the
//! S3's `Instant::now()`**.
//!
//! **The C6's view, verbatim, base as the only parameter** (`m6/notes.md`
//! §3.0 row 3): every register `+0x00..+0x8c` plus `date +0xfc` is
//! byte-identical between the two PACs (`regs::SYSTIMER` here, generated
//! from the S3's), and the single difference — `conf.etm_en` bit 1 — is
//! C6-only and nothing here ever read it. This is the cleanest reuse in the
//! milestone, and it is the copy that matters most, because on this chip
//! this block *is* the clock:
//!
//! - `esp_hal::time::Instant::now()` is `SystemTimer::unit_value(Unit0)`
//!   converted by `ticks_to_us` (`time.rs:775-781`, the
//!   `systimer_driver_supported` arm) — where the classic used TIMG0's
//!   LACT. `esp_rtos::now`, `TimeDriver::arm_next_wakeup`,
//!   `timer_tick_handler` and `Alarm::set_enable` all read through it.
//! - The rate is **XTAL / 2.5 = 16 MHz** (`timer/systimer.rs:195-215`:
//!   `xtal_clk_frequency * 10 / 25`), which at 240 MHz is one tick per
//!   **15 cycles** ([`CYCLES_PER_TICK`]). `init_timestamp_scaler`
//!   (`systimer.rs:114-134`) turns 16 ticks/µs into a shift by 4, so
//!   `micros = ticks >> 4`; `tests/clock.rs` pins that derivation.
//!
//! Register facts (offsets in `regs::SYSTIMER`):
//!
...
//!   `load_value` writes the period, commits, then flips `period_mode` off
//!   and on). `conf.targetN_work_en` arms the comparator; an armed one fires
//!   at the cycle where its unit reaches the committed target, and in period
//!   mode re-arms at `+period`.
//! - On fire: `int_raw.targetN`; the source `SYSTIMER_TARGET0+N` (57..=59,
//!   `regs::source`) is the level `int_raw & int_ena`. `int_clr` is
//!   write-1-to-clear, `int_st = int_raw & int_ena`.
//! - `conf` resets to `0x4600_0000`: Unit0 running, Unit1 stopped
//!   (`regs::SYSTIMER`'s one non-zero reset).
//!
...
//! surprise).
//!
//! The shipped firmware's *tick* is not here either — `esp_rtos::start` is
//! handed `timg0` (`fw-esp32s3/src/board/esp32s3/init.rs:69`), so the tick
//! is [`super::timg`]'s T0. This block serves `Instant::now`, `Delay`, and
//! `esp_rtos`'s `now()`.
...
/// The block's aperture: the generated table runs to `+0xfc` (`date`).
pub const SYSTIMER_LEN: u32 = 0x100;

/// XTAL / 2.5 (`timer/systimer.rs:212`).
pub const TICKS_PER_SECOND: u64 = 16_000_000;
/// One tick per fifteen CPU cycles at 240 MHz.
...
/// `unit_op` read with, and nothing else.
...
pub struct Systimer {
    index: usize,
    conf: u32,
    /// `count = (cycles / 15 + offset) & MASK52` while the unit runs.
    offset: [u64; 2],
...
impl Systimer {
    ...
    /// the **latched** value, which moves only on a `unit_op.update` write.
    ...
    fn rearm(&mut self, n: usize, cx: &mut BusCx<'_>) {
        ...
        // First cycle `c` with `c / 15 + offset >= target`.
...
/// A little-endian cursor for the state blobs, shared by the views in this
/// module (the C6 keeps its copy in the same place).
...
mod tests {
    ...
    /// esp-hal's one-shot `schedule` on a systimer alarm.
    ...
    /// The derivation this block exists for: 16 MHz from 240 MHz cycles,
    /// and `ticks >> 4` is microseconds.
    #[test]
    fn unit0_counts_at_sixteen_megahertz_from_cycles_and_unit1_is_stopped_at_reset() {
        ...
        assert_eq!(CYCLES_PER_TICK, 15);
        sb.now = 2_400_000; // 10 ms at 240 MHz
        assert_eq!(read_count(&mut sb, &mut t, 0), 160_000);
        assert_eq!(160_000 >> 4, 10_000, "esp-hal's shift: 10 000 us");
        sb.now = 2_400_015;
        ...
        // Start it: it counts from where it was, not from cycles/15.
    ...
    fn a_one_shot_alarm_fires_at_the_target_and_drives_the_source_while_enabled() {
        ...
        sb.now = 1500;
        schedule(&mut sb, &mut t, 0, 100); // fires at count 200 = cycle 3000
        assert_eq!(sb.sched.next_deadline(), Some(3000));
        assert_eq!(sb.read(&mut t, REAL_TARGET0_LO), 200);
        sb.run_to(&mut t, 2999);
        assert!(!sb.irq.level(source::SYSTIMER_TARGET0));
        sb.run_to(&mut t, 3000);
        assert!(sb.irq.level(source::SYSTIMER_TARGET0));
        assert_eq!(sb.read(&mut t, INT_RAW), 1);
        assert_eq!(sb.read(&mut t, INT_ST), 1);
        assert_eq!(sb.sched.next_deadline(), None, "one-shot");
    ...
    fn comp_load_cancels_and_re_arms_and_a_past_target_fires_at_once() {
        ...
        assert_eq!(sb.sched.next_deadline(), Some(16_500));
        sb.now = 3000;
        schedule(&mut sb, &mut t, 0, 10);
        assert_eq!(sb.sched.next_deadline(), Some(3150));
        ...
        assert_eq!(
            ...
            Some(7500),
            "due now, not in nine years"
        );
        let conf = sb.read(&mut t, CONF);
        sb.write(&mut t, CONF, conf & !TARGET_WORK_EN[0]);
        assert_eq!(sb.sched.next_deadline(), None, "disarming cancels");
    }
    ...
    fn period_mode_re_arms_itself_at_plus_period() {
        ...
        assert_eq!(sb.sched.next_deadline(), Some(750));
        sb.write(&mut t, INT_CLR, 4);
        sb.run_to(&mut t, 750);
        assert!(sb.irq.level(source::SYSTIMER_TARGET2));
        assert_eq!(sb.sched.next_deadline(), Some(1500));
    }
