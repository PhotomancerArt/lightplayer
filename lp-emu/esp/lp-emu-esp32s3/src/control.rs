...
//!
//! # What this chip's link cannot do, and still answers
//!
//! ⚠️ **`reset` and `download-mode` are unconditional here.** The S3's
//! `usb_device` block has no `chip_rst` — `0x4c`…`0x7c` is reserved
//! (`esp32s3-0.35.2/src/usb_device.rs:22`) — so there is no
//! `disable_usb_serial_chip_reset` bit for a guest to set, and the C6's
//! `err` reply naming that bit has no counterpart on this machine.
//!
//! ⚠️ **`dtr`, `rts` and `signals` reach no guest-visible register.** The
//! shipped image's `UsbConnectionMonitor::poll` reads `int_raw.sof` and
//! clears it and does nothing else
//! (`lp-fw/fw-esp32s3/src/board/esp32s3/usb_connection.rs:29-32`), and the
//! two `*_chg` interrupt bits the C6 raises are not declared on this part.
//! The verbs stay because a host really does assert those lines and
//! esptool's dances are made of them — P06's flasher path needs them — and
//! because the RTS falling edge is still what decodes a dance into a reset
//! or a download-mode request.
//!
//! ⚠️ **`pin` and `pins` are not here yet.** They are the pad fabric's, the
//! fabric is unconfigured on this machine until **P07**, and a verb that
//! drove a pad no chip has is worse than one that is honestly absent. The
//! parser refuses them by name and says which phase owns them, rather than
//! answering "unknown command".
//!
//! ⚠️ **The CH340 cable has no counterpart here.** The S3 is native USB:
//! `lp-emu-esp32v3/src/control.rs`'s auto-reset truth table is about a wire
//! between a USB-serial bridge and two pins of the chip, and this link is
//! the chip.
//!
//! # Time
//!
...
impl ControlCommand {
    ...
    /// Verbs the C6's channel has that this machine does not, and the phase
    /// that owns each. Refused **by name**, so a script written against the
    /// other machine says what is missing rather than "unknown command".
    pub const NOT_YET: &'static [(&'static str, &'static str)] = &[("pin", "P07"), ("pins", "P07")];
    ...
    pub fn parse(line: &str) -> Result<Self, String> {
        ...
        match verb {
            ...
            other => match Self::NOT_YET.iter().find(|(v, _)| *v == other) {
                Some((verb, phase)) => Err(format!(
                    "`{verb}` is the pad fabric's and this machine has no pads until M6 \
                     {phase} — refused by name rather than silently accepted"
                )),
                None => Err(format!(
...
mod tests {
    ...
    fn a_reply_names_the_command_and_the_guest_cycle_it_took_effect_at() {
        // ⚠️ The microseconds are THIS chip's: 240 cycles to the microsecond,
        // not the C6's 160. A reply copied across without the division
        // would be a plausible-looking lie about when a command landed.
        ...
        assert_eq!(
            ...
            .to_string(),
            "ok state cyc=240000 us=1000 host=attached draining=true sof=on in_pending=64 \
             out_queued=0"
        );
        assert_eq!(
            ControlReply::State {
    ...
    /// The two verbs the C6's channel has and this machine does not are
    /// refused **by name**, with the phase that owns them. A script written
    /// against the other machine then says what is missing.
    #[test]
    fn the_pad_verbs_are_refused_by_name_and_say_which_phase_owns_them() {
        for line in ["pin 20 1", "pins", "pin gpio21 0"] {
            let err = ControlCommand::parse(line).unwrap_err();
            assert!(err.contains("pad fabric"), "`{line}` gave {err:?}");
            assert!(err.contains("P07"), "`{line}` gave {err:?}");
            assert!(
                !err.contains("unknown command"),
                "`{line}` must not read as a typo"
            );
        }
        assert!(!ControlCommand::VERBS.contains(&"pin"));
        assert!(!ControlCommand::VERBS.contains(&"pins"));
        // And a genuinely unknown word still lists what exists.
        let err = ControlCommand::parse("nope").unwrap_err();
        assert!(err.contains("unknown command"), "{err}");
    }

    /// `reset` and `download-mode` still parse — they are unconditional on
    /// this chip rather than absent, and the difference is the README's.
    #[test]
    fn the_reset_verbs_parse_even_though_the_guest_cannot_refuse_them() {
        assert_eq!(
            ControlCommand::parse("reset").unwrap(),
            ControlCommand::Reset
        );
        assert_eq!(
            ControlCommand::parse("download-mode").unwrap(),
            ControlCommand::DownloadMode
        );
    }
