...
| `COEX` | `0x600A_F400..F800` | accept | the Wi-Fi/BLE **coexistence arbiter**, not in the PAC; the coex blob's `coex_hw_clear_reg` writes it at Wi-Fi init in any image that links `esp-radio/coex` — the shipped one since BLE M4 (the strict-bus stop that found it: `W4 0x600af400`). Remembered, never read back so far. No emulated board turns BLE on, so the arbiter never has two radios to arbitrate |
