...
| `project-access.schema.json` | JSON Schema for the project access sidecar `<project>/.lp/access.json` (`lpc_access::ProjectAccessFile`). Its **own** persisted format with its own `"version": 1`, outside `PROJECT_FORMAT_VERSION`: `/.lp/` is not part of the project format (hash-excluded, skipped by publish/fork/zip export, write-only over every link). A serde change to it bumps its `VERSION` and ships the reader for the old one with it. |
| `device-access.schema.json` | JSON Schema for the device access store, root `/.lp/access.json` on a device (`lpc_access::DeviceAccessFile`): device-only secrets, the account default, `bleEnabled`, `open`. Own format, `"version": 1`, same rule as the sidecar. |
