...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Generate jit_builtin_ptr.rs (BuiltinId -> code address)
    let jit_builtin_ptr_path = workspace_root
        .join("lps-builtins")
        .join("src")
        .join("jit_builtin_ptr.rs");
    generate_jit_builtin_ptr(&jit_builtin_ptr_path, &builtins);
...
fn generate_glsl_builtin_mapping(
    ...
    let header = r#"//! GLSL / LPIR / LPFX name → `BuiltinId`, per float mode.
//!
...
"#;
    ...
    // Every family is emitted once per mode. A builtin belongs to a mode when
    // its symbol carries that suffix, or when it carries no suffix at all —
    // `lpfn_hash_*` is genuinely mode-independent (integer in, integer out) and
    // is the correct answer in either mode.
    for mode in [BuiltinMode::Q32, BuiltinMode::F32] {
        emit_glsl_math_map(&mut out, builtins, mode);
        emit_lpir_map(&mut out, builtins, mode);
        emit_vm_map(&mut out, builtins, mode);
        emit_texture_map(&mut out, builtins, mode);
        emit_lpfn_map(&mut out, &parsed, mode);
    }

    emit_mode_facades(&mut out);
...
/// Which float mode a generated resolver serves.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum BuiltinMode {
    Q32,
    F32,
}

impl BuiltinMode {
    /// `q32` / `f32` — the symbol suffix and the resolver-name infix.
    ...
    /// A builtin serves this mode when it carries the matching suffix, or no
    /// suffix at all. Mode-independent builtins (`lpfn_hash_*`: integer in,
    /// integer out) are the correct answer in both modes, and omitting them
    /// from the f32 map would make every `lpfn_hash` call unresolvable in
    /// Float mode.
    fn accepts(self, builtin_mode: Option<&str>) -> bool {
        match builtin_mode {
            None => true,
            Some(m) => m == self.tag(),
        }
...
fn emit_glsl_math_map(out: &mut String, builtins: &[BuiltinInfo], mode: BuiltinMode) {
    let tag = mode.tag();
    out.push_str(&format!(
        "/// Map `@glsl::*` import name + **AST argument count** to a {tag} scalar builtin.\n\
         /// Returns `None` for builtins implemented inline in WASM (e.g. `clamp`) or unknown names.\n\
         pub fn glsl_{tag}_math_builtin_id(name: &str, arg_count: usize) -> Option<BuiltinId> {{\n\
         match (name, arg_count) {{\n"
    ));
    for builtin in builtins {
        if builtin.builtin_module != "glsl" || !mode.accepts(builtin.builtin_mode.as_deref()) {
            continue;
        }
...
fn emit_lpir_map(out: &mut String, builtins: &[BuiltinInfo], mode: BuiltinMode) {
    let tag = mode.tag();
    out.push_str(&format!(
        "/// Map `@lpir::*` import name + argument count to a {tag} builtin.\n\
         pub fn lpir_{tag}_builtin_id(name: &str, arg_count: usize) -> Option<BuiltinId> {{\n\
         match (name, arg_count) {{\n"
    ));
    for builtin in builtins {
        if builtin.builtin_module != "lpir" || !mode.accepts(builtin.builtin_mode.as_deref()) {
            continue;
        }
        // The import is spelled `@lpir::sqrt`; the symbol is `fsqrt`.
...
fn emit_vm_map(out: &mut String, builtins: &[BuiltinInfo], mode: BuiltinMode) {
    let tag = mode.tag();
    out.push_str(&format!(
        "/// Map `@vm::*` import name + user-visible argument count to a {tag} builtin.\n\
         pub fn vm_{tag}_builtin_id(name: &str, arg_count: usize) -> Option<BuiltinId> {{\n\
         match (name, arg_count) {{\n"
    ));
    for builtin in builtins {
        if builtin.builtin_module != "vm" || !mode.accepts(builtin.builtin_mode.as_deref()) {
            continue;
        }
...
fn emit_texture_map(out: &mut String, builtins: &[BuiltinInfo], mode: BuiltinMode) {
    let tag = mode.tag();
    out.push_str(&format!(
        "/// Map `@texture::*` import base name + user-visible argument count to a {tag} builtin.\n\
         /// Frontend may suffix the name with `_<naga id>` — strip that suffix before lookup.\n\
         pub fn texture_{tag}_builtin_id(name: &str, arg_count: usize) -> Option<BuiltinId> {{\n\
         match (name, arg_count) {{\n"
    ));
    for builtin in builtins {
        if builtin.builtin_module != "texture" || !mode.accepts(builtin.builtin_mode.as_deref()) {
            continue;
        }
        out.push_str(&format!(
            "        (\"{}\", {}) => Some(BuiltinId::{}),\n",
...
fn emit_lpfn_map(out: &mut String, parsed: &[ParsedLpfnFunction], mode: BuiltinMode) {
    let tag = mode.tag();
    out.push_str(&format!(
        "/// Map `lpfn_*` name + parameter type list to the {tag} `BuiltinId`.\n\
         pub fn glsl_lpfn_{tag}_builtin_id(name: &str, params: &[GlslParamKind]) -> Option<BuiltinId> {{\n\
         match (name, params) {{\n"
    ));
    ...
    for glsl_name in sorted_names {
        ...
        for (signature_funcs, sig) in sigs {
            let Some(variant) = lpfn_builtin_variant(&signature_funcs, mode) else {
                continue;
            };
...
///
/// Hand-written backends kept re-deriving "which resolver for which mode", and
/// the wasm emitter got it wrong in a way validation could not catch. One
/// generated dispatch point means a new float mode is one edit here.
fn emit_mode_facades(out: &mut String) {
    for (facade, q32, f32, args, call) in [
        (
            "glsl_math_builtin_id",
            "glsl_q32_math_builtin_id",
            "glsl_f32_math_builtin_id",
            "name: &str, arg_count: usize",
            "name, arg_count",
        ),
        (
            "lpir_builtin_id",
        ...
        (
            ...
            "texture_f32_builtin_id",
            "name: &str, arg_count: usize",
            "name, arg_count",
        ),
        (
            "glsl_lpfn_builtin_id",
            "glsl_lpfn_q32_builtin_id",
            "glsl_lpfn_f32_builtin_id",
            "name: &str, params: &[GlslParamKind]",
            "name, params",
        ),
    ] {
        out.push_str(&format!(
            "/// Resolve for `mode`. Never falls back across modes — see the module docs.\n\
             pub fn {facade}({args}, mode: Mode) -> Option<BuiltinId> {{\n\
             \x20   match mode {{\n\
             \x20       Mode::Q32 => {q32}({call}),\n\
             \x20       Mode::F32 => {f32}({call}),\n\
             \x20   }}\n\
             }}\n\n"
        ));
    }
...
/// Tests that fail loudly if a regeneration silently drops a family or a mode.
fn emit_mapping_tests(out: &mut String, builtins: &[BuiltinInfo]) {
    let variant = |pred: &dyn Fn(&BuiltinInfo) -> bool, what: &str| -> String {
        builtins
            .iter()
            .find(|b| pred(b))
            .map(|b| b.enum_variant.clone())
            .unwrap_or_else(|| panic!("missing builtin for generated test: {what}"))
    };

    let sin_q32 = variant(
        &|b| b.builtin_module == "glsl" && b.builtin_fn_name == "sin" && is_mode(b, "q32"),
        "glsl sin q32",
    );
    let atan2_q32 = variant(
        &|b| b.builtin_module == "glsl" && b.builtin_fn_name == "atan2" && is_mode(b, "q32"),
        "glsl atan2 q32",
    );
    let sqrt_q32 = variant(
        &|b| b.builtin_module == "lpir" && b.builtin_fn_name == "fsqrt" && is_mode(b, "q32"),
        "lpir fsqrt q32",
    );
    let sqrt_f32 = variant(
        &|b| b.builtin_module == "lpir" && b.builtin_fn_name == "fsqrt" && is_mode(b, "f32"),
        "lpir fsqrt f32",
    );
    let get_fuel = variant(
        &|b| b.builtin_module == "vm" && b.builtin_fn_name == "get_fuel",
        "vm get_fuel",
    );
    let texture_2d_q32 = variant(
        &|b| b.symbol_name == "__lp_texture2d_rgba16_unorm_q32",
        "texture2d rgba16 unorm q32",
    );
    let fbm_q32 = variant(&|b| b.symbol_name == "__lp_lpfn_fbm2_q32", "lpfn fbm2 q32");
    let fbm_f32 = variant(&|b| b.symbol_name == "__lp_lpfn_fbm2_f32", "lpfn fbm2 f32");
    let hash_1 = variant(&|b| b.symbol_name == "__lp_lpfn_hash_1", "lpfn hash_1");
...
mod glsl_builtin_mapping_tests {{
    ...
    #[test]
    fn q32_sin() {{
        assert_eq!(glsl_q32_math_builtin_id("sin", 1), Some(BuiltinId::{sin_q32}));
    }}

    #[test]
    fn q32_atan_two_args_is_atan2_import() {{
        assert_eq!(glsl_q32_math_builtin_id("atan", 2), Some(BuiltinId::{atan2_q32}));
    }}

    #[test]
    fn lpir_sqrt() {{
        assert_eq!(lpir_q32_builtin_id("sqrt", 1), Some(BuiltinId::{sqrt_q32}));
    }}

    #[test]
    fn vm_get_fuel() {{
        assert_eq!(vm_q32_builtin_id("__lp_get_fuel", 0), Some(BuiltinId::{get_fuel}));
    }}
    ...
    fn lpfn_fbm_vec2() {{
        assert_eq!(
            ...
            Some(BuiltinId::{fbm_q32})
        );
    }}

    #[test]
    fn f32_resolvers_exist_for_every_family() {{
        assert_eq!(lpir_f32_builtin_id("sqrt", 1), Some(BuiltinId::{sqrt_f32}));
        assert_eq!(
            glsl_lpfn_f32_builtin_id(
                "lpfn_fbm",
                &[GlslParamKind::Vec2, GlslParamKind::Int, GlslParamKind::UInt],
            ),
            Some(BuiltinId::{fbm_f32})
        );
    }}
    ...
    fn mode_independent_builtins_resolve_in_both_modes() {{
        ...
        assert_eq!(glsl_lpfn_q32_builtin_id("lpfn_hash", params), Some(BuiltinId::{hash_1}));
        assert_eq!(glsl_lpfn_f32_builtin_id("lpfn_hash", params), Some(BuiltinId::{hash_1}));
    }}
...
fn is_mode(b: &BuiltinInfo, tag: &str) -> bool {
    b.builtin_mode.as_deref() == Some(tag)
}
...
/// The `BuiltinId` variant for one lpfn overload in `mode`.
///
/// An overload that declares mode variants (`#[lpfn_impl(q32, …)]` /
/// `#[lpfn_impl(f32, …)]`) resolves to the matching one. An overload with no
/// declared variant is mode-independent (`lpfn_hash_*`) and resolves to itself
/// in both modes.
fn lpfn_builtin_variant(funcs: &[&ParsedLpfnFunction], mode: BuiltinMode) -> Option<String> {
    let wanted = match mode {
        BuiltinMode::Q32 => Variant::Q32,
        BuiltinMode::F32 => Variant::F32,
    };
    let has_decimal = funcs.iter().any(|f| f.attribute.variant.is_some());
    if has_decimal {
        funcs
            .iter()
            .find(|f| f.attribute.variant == Some(wanted))
...
"#;
    ...
    // Mode-taking facades — what backends should call.
    output.push_str("pub use glsl_builtin_mapping::glsl_lpfn_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::glsl_math_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::lpir_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::texture_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::vm_builtin_id;\n");
    // Per-mode resolvers — kept public for call sites that are statically
    // single-mode (Q32-only native lowering, completions drift tests).
    output.push_str("pub use glsl_builtin_mapping::glsl_lpfn_f32_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::glsl_lpfn_q32_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::glsl_f32_math_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::glsl_q32_math_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::lpir_f32_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::lpir_q32_builtin_id;\n");
    output.push_str("pub use glsl_builtin_mapping::texture_f32_builtin_id;\n");
...
"#;
    ...
    if builtins.is_empty() {
        ...
        for module_path in modules {
            ...
            // A `use` statement covers exactly one source file, so the whole
            // statement is q32 or f32 — never mixed.
            if components.last().is_some_and(|c| file_is_f32(c)) {
    ...
    for builtin in builtins {
        ...
        if !f32_cfg_attr(builtin.builtin_mode.as_deref()).is_empty() {
            output.push_str("        #[cfg(feature = \"float-f32\")]\n");
        }
        output.push_str(&format!(
...
///
/// The f32 builtins are feature-gated in `lps-builtins` (roadmap D2) so a
/// Fixed-mode-only device image links none of them. Every generated file that
/// *names* an f32 symbol has to carry the gate, or the crate fails to compile
/// with the feature off — which is exactly the configuration the ESP32-C6
/// harnesses build.
fn f32_cfg_attr(builtin_mode: Option<&str>) -> &'static str {
    match builtin_mode {
        Some("f32") => "#[cfg(feature = \"float-f32\")]\n",
        _ => "",
    }
}

/// True when `file_name` holds native-f32 builtins (`*_f32.rs`).
fn file_is_f32(file_name: &str) -> bool {
    file_name.ends_with("_f32")
}

/// `BuiltinId` → machine-code address, for JIT relocation targets.
///
/// This used to be hand-maintained with a "keep in sync" comment at the top,
/// which is a standing tax that the f32 family would have doubled. Every field
/// it needs — enum variant, module path, symbol name — is already scraped, so
/// it is generated and the drift class is gone.
fn generate_jit_builtin_ptr(path: &Path, builtins: &[BuiltinInfo]) {
