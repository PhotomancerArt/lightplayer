//! Native f32 `tanh` — the f32 sibling of [`super::tanh_q32`].
//!
//! **Implementation:** `libm::tanhf`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Saturates to `±1` for large `|x|`.

/// GLSL `tanh(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_tanh_f32(x: f32) -> f32 {
    libm::tanhf(x)
}
