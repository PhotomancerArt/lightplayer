//! Native f32 `atanh` — the f32 sibling of [`super::atanh_q32`].
//!
//! **Implementation:** `libm::atanhf`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Defined on `(-1, 1)`. At or outside `±1` GLSL is undefined (float.md §5).

/// GLSL `atanh(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_atanh_f32(x: f32) -> f32 {
    libm::atanhf(x)
}
...
mod tests {
    ...
    fn within_the_declared_band_over_the_sample_range() {
        for i in -40..=40 {
            let x = i as f32 * 0.1;
            let got = __lps_atanh_f32(x);
            let want = (x as f64).atanh() as f32;
            if !want.is_finite() {
                // float.md §5: out-of-domain results are Unspecified — never asserted.
                continue;
            }
            assert_f32_close(got, want, 1e-6, 1e-6, format_args!("atanh({x})"));
        }
    }
}
