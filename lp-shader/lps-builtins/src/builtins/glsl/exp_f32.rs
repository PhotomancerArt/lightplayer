//! Native f32 `exp` — the f32 sibling of [`super::exp_q32`].
//!
//! **Implementation:** `libm::expf`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Overflows to `inf` past ~88.7, underflows to `0` below ~-103.

/// GLSL `exp(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_exp_f32(x: f32) -> f32 {
    libm::expf(x)
}
