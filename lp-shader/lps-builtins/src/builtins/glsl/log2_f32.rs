//! Native f32 `log2` — the f32 sibling of [`super::log2_q32`].
//!
//! **Implementation:** `libm::log2f`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Defined for `x > 0`; exact on powers of two.

/// GLSL `log2(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_log2_f32(x: f32) -> f32 {
    libm::log2f(x)
}
