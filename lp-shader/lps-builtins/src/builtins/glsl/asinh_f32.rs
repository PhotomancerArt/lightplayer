//! Native f32 `asinh` — the f32 sibling of [`super::asinh_q32`].
//!
//! **Implementation:** `libm::asinhf`. `libm` is a `no_std` workspace
...
/// GLSL `asinh(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_asinh_f32(x: f32) -> f32 {
    libm::asinhf(x)
}
...
mod tests {
    ...
    fn within_the_declared_band_over_the_sample_range() {
        for i in -40..=40 {
            let x = i as f32 * 0.1;
            let got = __lps_asinh_f32(x);
            let want = (x as f64).asinh() as f32;
            if !want.is_finite() {
                // float.md §5: out-of-domain results are Unspecified — never asserted.
                continue;
            }
            assert_f32_close(got, want, 1e-6, 1e-6, format_args!("asinh({x})"));
        }
    }
}
