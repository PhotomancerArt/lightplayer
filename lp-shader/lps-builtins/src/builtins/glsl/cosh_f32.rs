//! Native f32 `cosh` — the f32 sibling of [`super::cosh_q32`].
//!
//! **Implementation:** `libm::coshf`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Overflows to `inf` past ~89.

/// GLSL `cosh(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_cosh_f32(x: f32) -> f32 {
    libm::coshf(x)
}
