//! Native f32 `log` — the f32 sibling of [`super::log_q32`].
//!
//! **Implementation:** `libm::logf`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Defined for `x > 0`. `log(0)` is `-inf` and `log(x < 0)` is NaN; GLSL leaves both undefined (float.md §5).

/// GLSL `log(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_log_f32(x: f32) -> f32 {
    libm::logf(x)
}
...
mod tests {
    ...
    fn known_values() {
        ...
        assert_f32_close(
            __lps_log_f32(core::f32::consts::E),
            1.0,
            1e-6,
            1e-6,
            format_args!("log(core::f32::consts::E)"),
        );
    }
