//! Native f32 `tan` — the f32 sibling of [`super::tan_q32`].
//!
//! **Implementation:** `libm::tanf`. `libm` is a `no_std` workspace
...
//!
//! **Domain:** Poles at odd multiples of pi/2 return a huge finite value or inf; GLSL leaves it undefined (float.md §5).

/// GLSL `tan(x)`.
#[unsafe(no_mangle)]
pub extern "C" fn __lps_tan_f32(x: f32) -> f32 {
    libm::tanf(x)
}
...
mod tests {
    ...
    fn known_values() {
        ...
        assert_f32_close(
            __lps_tan_f32(core::f32::consts::FRAC_PI_4),
            1.0,
            1e-6,
            1e-6,
            format_args!("tan(core::f32::consts::FRAC_PI_4)"),
        );
    }
    ...
    fn within_the_declared_band_over_the_sample_range() {
        for i in -40..=40 {
            let x = i as f32 * 0.1;
            let got = __lps_tan_f32(x);
            let want = (x as f64).tan() as f32;
            if !want.is_finite() {
                // float.md §5: out-of-domain results are Unspecified — never asserted.
                continue;
            }
            assert_f32_close(got, want, 1e-6, 1e-6, format_args!("tan({x})"));
        }
    }
}
