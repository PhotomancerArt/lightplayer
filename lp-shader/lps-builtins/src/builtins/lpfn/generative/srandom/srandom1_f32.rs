//! 1D signed random in `[-1, 1]` (native f32).
//!
//! Transliterated from the canonical GLSL
//! `glsl/lpfn/generative/srandom/srandom1.glsl` (normative): a trivial
...
/// 1D signed random function (float version).
///
/// # Returns
...
pub extern "C" fn __lp_lpfn_srandom1_f32(x: f32, seed: u32) -> f32 {
    -1.0 + 2.0
        * crate::builtins::lpfn::generative::random::random1_f32::__lp_lpfn_random1_f32(x, seed)
}
...
mod tests {
    ...
    fn output_is_signed_and_bounded() {
        for i in -200..=200 {
            let t = i as f32 * 0.23;
            let v = __lp_lpfn_srandom1_f32(t, 0);
            assert!((-1.0..1.0).contains(&v), "srandom1({t}) = {v}");
        }
    }
    ...
    fn it_is_exactly_the_unsigned_form_remapped() {
        let t = 1.75f32;
        let u = crate::builtins::lpfn::generative::random::random1_f32::__lp_lpfn_random1_f32(t, 0);
        assert_eq!(__lp_lpfn_srandom1_f32(t, 0), -1.0 + 2.0 * u);
    }
}
