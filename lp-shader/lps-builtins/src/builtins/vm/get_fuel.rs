...
//!
//! **Mode-independent**, and named so. Its ABI is `(i32) -> u32` — no float
//! crosses the boundary in either direction, so there is one implementation for
//! both Fixed and Float mode, exactly like `lpfn_hash_*`. It used to carry a
//! `_q32` suffix, which made the f32 resolver miss it and would have silently
//! dropped fuel metering from every Float-mode shader.

use lpvm::VmContext;

#[unsafe(no_mangle)]
pub extern "C" fn __lp_vm_get_fuel(vmctx_word: i32) -> u32 {
