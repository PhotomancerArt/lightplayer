...
| `EmuCore` trait over arch emulators (consumers call concrete `Emulator` methods; the gate is an arch-neutral replacement for cranelift's `Signature` in `call_function*`) | `2026-07-28-emu-core-crate-family` | `lp-xt-emu` lands in-repo and a consumer needs arch polymorphism |
