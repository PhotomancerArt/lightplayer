...
- **Status:** Rejected
- **Date:** 2026-09-08
- **Deciders:** Photomancer (Yona); ADR gate passed 2026-09-08 00:15, ruling
  reversed 2026-09-08 22:16
- **Supersedes:** None
- **Superseded by:** None

Plan: `lp2025/2026-09-07-0827-emu-speed-ladder`, milestone M4, decision D7.
PR: [#597](https://github.com/PhotomancerArt/lightplayer/pull/597), closed
unmerged. Branch `claude/emu-speed-m4-poll-loop-skip` (head `e46a7b769`) is
kept on the remote — its tests, its identity-oracle script and its
boundary-walk test are reusable, most immediately by M5.

## Why rejected

The design below was built, gated and measured — this ADR is that record —
and then reversed on evidence that arrived after the gate. **Abandoned, with
cause** (Yona, 2026-09-08 22:16: *"good experiment"*).

The ADR gate (00:15) approved the design knowing the one measured cost:
`boot-idle-memfs`, a `wfi`-idle boot with almost no console traffic, ran
11–19 % slower with the skip on, because the skip never fires there and the
per-instruction bookkeeping still runs. That looked like an edge case — every
image that logs over UART gained 2.4×–6×, and idle-and-silent seemed like the
only skip-free shape worth worrying about.

It was not the only shape. M5 P0 added two reference images built from the
product's real render loop (`render-basic`, 241 lamps with a JIT'd shader
over RMT; `render-rocaille`, 4× the shader) — **busy and silent**, the class
nothing had measured yet. Both lost:

| image | grade | `main` | skip **on** | vs main | skip **off** | vs main |
|---|---|---:|---:|---:|---:|---:|
| harness | t1 | 3.85 | **0.65** | **−83.1 % (5.9×)** | 4.07 | +5.7 % |
| harness | t2 | 3.50 | **0.66** | **−81.1 % (5.3×)** | 3.65 | +4.3 % |
| jit-math-perf | t1 | 2.10 | **0.78** | **−62.9 % (2.7×)** | 2.20 | +4.8 % |
| jit-math-perf | t2 | 1.42 | **0.60** | **−57.7 % (2.4×)** | 1.59 | +12.0 % |
| render-basic | t1 | 7.00 | 7.81 | **+11.6 %** | 7.36 | +5.1 % |
| render-basic | t2 | 6.66 | 7.26 | **+9.0 %** | 6.86 | +3.0 % |
| render-rocaille | t1 | 6.43 | 6.86 | **+6.7 %** | 6.75 | +5.0 % |
| render-rocaille | t2 | 7.61 | 8.22 | **+8.0 %** | 8.05 | +5.8 % |
| boot-idle-memfs | t1 | 0.75 | 0.89 | **+18.7 %** | 0.79 | +5.3 % |
| boot-idle-memfs | t2 | 0.70 | 0.77 | **+10.0 %** | 0.73 | +4.3 % |

(Interleaved round-robin, minimum USER seconds over 3 rounds, same window
per row. Full table and method:
[PR comment](https://github.com/PhotomancerArt/lightplayer/pull/597#issuecomment-5589352001).)

Every gaining image gains for the same reason — console drain, because
`harness` and `jit-math-perf` both log everything over UART at baud.
`render-basic` and `render-rocaille` are busy (a JIT'd shader runs every
frame) but silent (nothing goes over UART), and the detector never once
reaches a fixed point on them: instrumented, `render-basic` t2 shows 202,289
arms and **zero credits** over the whole 5.5 s run. The 5.9× on `harness` was
always a logging number, not a workload number — the PR body said as much on
day one — and the render loop is where that distinction stopped being
academic.

**Default-off is not the cheap escape it looks like, either.** The last
column above — `--no-poll-skip` on this branch's own binary — is +3.0 % to
+5.8 % slower than `main` on *every* image, including the ones that gain the
most with the skip on. The reason: `--no-poll-skip` only stops the *hart*
from taking a `PollSample`; `SocBus` still computes and stores one on every
access regardless. Shipping default-off gets a uniform ~5 % tax and no gain
anywhere — strictly worse than not merging at all.

**The cost is code shape, not work, and five bounded attempts could not
recover it:**

| tried | recovered |
|---|---|
| arm the detector only on a pure read, not on every load | nothing |
| fixed-size array compare instead of a slice `memcmp` on the store path | nothing |
| carry the horizon in the hart instead of as a `step` argument | nothing |
| box the detector so `MachineHart` does not hold a second register file | nothing |
| make `--no-poll-skip` skip the bus-side `PollSample` recording too (`bc5411500`: `SocBus::poll_sampling`, guarding the store-path compare, the `pure_read` call and the per-instruction reset) | isolated and measured against the same tree with it reverted: better on two rows, worse on two — below the noise floor at this desk's load |

What is left after all five is diffuse: an extra branch in `step` and a wider
`Bus` trait, live in the generated code whether or not any poll logic ever
executes. No one line owns it, so no runtime flag can skip it.

Two engineering fixes aimed at the render-loop finding specifically were also
tried and refuted before the residue work above:

- **A horizon gate at the arm point** (decline to arm when the next
  scheduled event is close). Refuted by measurement: the render images'
  horizon averages 176 million cycles — about 1.1 s at 160 MHz, not the
  microseconds the hypothesis needed — and more to the point, the detector
  never reaches a fixed point on them at all (`fixed-point reached: 0`), so
  there is no horizon-shaped saving to gate.
- **A fruitless-arm disarm** (give up sampling after N arms with no credit).
  Refuted: the harness's own longest arm streak before its first credit
  reaches 1,015,768, while the render images arm only 202,289 times in an
  *entire run* with zero credits ever. No threshold separates the two
  populations, so the disarm can never trigger where it is needed without
  also killing the harness's gain.

**Where this goes next.** A poll loop in a block-cache design is a
self-looping block — the same fixed-point argument (same `pc`, same register
file, no observable store) is a property of a *block*, checked once at the
block's own boundary instead of on every instruction inside it, which is
where the per-instruction bookkeeping cost above stops being paid at all. The
feature returns as a block-boundary check at a fraction of this design's
cost, deferred as a follow-on to M5 (`lp2025/2026-09-07-0827-emu-speed-ladder`,
`m5-block-cache.md`) rather than reattempted as a standalone flag.

The identity property this design proved — a skip that credits `n` whole
iterations without executing them is byte-identical to executing them, down
to the register file and the UART transcript — is not in question and is not
what was rejected. **81/81 artefacts byte-identical**, five images at both
grades in both modes, against `origin/main`. What was rejected is paying its
bookkeeping cost everywhere in exchange for a win that, measured against the
product's actual workload, is a net loss.

The design record below is preserved as written at the ADR gate (00:15),
before the render-loop evidence existed, because it is still the correct
description of the mechanism and the reasoning a block-boundary version will
reuse.
