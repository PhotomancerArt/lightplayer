...
| --- | --- | --- | --- | --- |
| budget-exhaustion | 2026-09-24 | [ble-enabled-c6-refuses-a-project-switch-after-the-heap-cut](2026-09-24-ble-enabled-c6-refuses-a-project-switch-after-the-heap-cut.md) | open | fw-esp32c6 RAM split × lpa-server load gate: with BLE enabled on the 236,000 B main region, choker → zook is refused (largest free block 65,534 < 65,536) though 212 KB is free; 260,000 B accepts the same sequence |
| assumed-context | 2026-09-23 | [bluefy-hidden-page-does-not-see-ble-drops](2026-09-23-bluefy-hidden-page-does-not-see-ble-drops.md) | **open** | Studio's Web Bluetooth link (M5; `spikes/ble-lab` today): iOS suspends a hidden Bluefy page, so a BLE drop while the phone is locked is delivered only when the page is shown again, and SSE control streams close. The link itself survives a 30 s lock; reconnect needs no tap (803–954 ms) |
| assumed-context | 2026-09-23 | [five-wires-reads-a-report-still-on-the-uart-as-a-lost-one](2026-09-23-five-wires-reads-a-report-still-on-the-uart-as-a-lost-one.md) | fixed (`a3d928d4d`) | lp-emu-esp32v3 tests/five_wires.rs: the "no summary line lost" bound assumes every report is on the wire at the 10 s deadline; on the #794-over-lean-wire image the deadline lands 2 frames after frame 1980 with its five report lines still draining UART0 at 921,600 baud, and the test fails with nothing lost |
