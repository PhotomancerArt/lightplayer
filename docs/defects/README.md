...
| --- | --- | --- | --- | --- |
| untested-path | 2026-09-25 | [a-knob-jump-over-bluetooth-kills-the-c6-ble-host](2026-09-25-a-knob-jump-over-bluetooth-kills-the-c6-ble-host.md) | **open** | fw-esp32c6 ble_task × esp-radio controller × Studio Play over `ble:`: the Scale knob sent to its end (`End`, 1 → 4) is followed by `[hci] error parsing packet` and `[ble] host runner error — restarting it`; the link drops, and the restarted host never advertises again, so the piece is unreachable over Bluetooth until a USB reboot. 2 of 2 on the M7 laptop walk (Run M); a one-step knob change and a brightness change went through |
| state-conflation | 2026-09-25 | [turn-on-bluetooth-reports-a-timeout-the-board-answered](2026-09-25-turn-on-bluetooth-reports-a-timeout-the-board-answered.md) | **open** | lpa-studio-core access write × the link's one `ConversationInbox` × the card's frame feed: Turn on Bluetooth says `device did not respond within 5.0s` (4 of 4) while a wire tap shows the board's `error:null` reply 192 ms later. The feed, polling the same inbox, pops and drops the reply. The store did land; a manual Reset brings BLE up |
...
| assumed-context | 2026-09-23 | [five-wires-reads-a-report-still-on-the-uart-as-a-lost-one](2026-09-23-five-wires-reads-a-report-still-on-the-uart-as-a-lost-one.md) | fixed (`a3d928d4d`; recurrence 2026-09-24 fixed in PR #810) | lp-emu-esp32v3 tests/five_wires.rs: the "no summary line lost" bound assumes every report is on the wire at the 10 s deadline; on the #794-over-lean-wire image the deadline lands 2 frames after frame 1980 with its five report lines still draining UART0 at 921,600 baud, and the test fails with nothing lost |
...
| backend-contract-divergence | 2026-09-13 | [the-s3-link-drops-the-io-tasks-next-chunk-on-a-stale-serial-in-empty](2026-09-13-the-s3-link-drops-the-io-tasks-next-chunk-on-a-stale-serial-in-empty.md) | fixed | fw-esp32s3 io_task (fw-esp32c6 latent, follow-up) × esp-hal 1.1.1 write_async: two writers on USB-Serial-JTAG's one send buffer; esp-hal writes without checking it is free and wakes on a stale `serial_in_empty`. The link model was right (the TRM: refused while pending); fixed by a firmware gate, upstream issue drafted |
