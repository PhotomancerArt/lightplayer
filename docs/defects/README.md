...
| assumed-context | 2026-09-10 | [the-app-cores-rom-boot-rewrites-heap-region-0](2026-09-10-the-app-cores-rom-boot-rewrites-heap-region-0.md) | **open** | fw-esp32v3 heap region 0 (`0x3ffe0440..0x3ffe3f20`) registered before `start_app_core_isr`; the mask ROM's APP-core reset re-unpacks `.data_xtos_pro` and zeroes `.bss_xtos_pro` over it — found on the classic emulator (M4 P1), where the shipped image dies in `LpFs::read_file`; the desk board's survival is the open question |
