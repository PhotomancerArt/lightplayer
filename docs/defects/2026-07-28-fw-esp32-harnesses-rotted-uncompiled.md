...
fixed: this change
area: lp-fw/fw-esp32 (src/tests/ harnesses + module cfg gates)
...
`build-fw-esp32` and `clippy-fw-esp32` both build only `--features esp32c6`, and
...
`just clippy-fw-esp32-harnesses`, which clippies all thirteen harness feature
...
**Regression coverage** — `just clippy-fw-esp32-harnesses` (13 combos; ~13s
...
findable without any bug — thirteen `fwtest-*` recipes, one `build-fw-esp32`.
