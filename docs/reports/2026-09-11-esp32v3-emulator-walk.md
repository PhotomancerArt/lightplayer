...
| **The quantum, in the sidecar** | M5's brief specified a `quantum` field on the transcript header and **there is none** — `TranscriptHeader` has no such key and no classic sidecar states one. Every run in this record and every committed classic transcript is at the default **256**, recoverable from each sidecar's `source` command line (no `--core-quantum` in it) and printed in the run summary (`quantum=256`) that does not go into the transcript. Named rather than assumed; the field is a schema change and this phase did not make one. |
