...
| [emu-c6-radio-tx-never-completes](emu-c6-radio-tx-never-completes.md) | retired | 2026-09-08 | lp-emu-esp32c6 radio window (`periph/wifi_stub.rs`) | a WiFi TX armed on the emulated C6 never completed, wedging any image whose forward progress depends on `send(...).wait()`; retired 2026-09-08 by M4 U1 — the ISR dispatches on `WIFI_MAC+0x4c48` bit 7, gated by `+0x4cb8` bit 0, cleared through `+0x4cb4`, all `modeled` and unwitnessed by silicon |
| [emulator-heap-ledger-differs-from-silicon-by-eight-bytes](emulator-heap-ledger-differs-from-silicon-by-eight-bytes.md) | carried | 2026-09-07 | lp-emu-esp32c6 boot + fw-esp32c6 allocator | every capture, every link, every boot path: the emulator's idle heap reports `freeBytes` +8 / `usedBytes` −8 against silicon; sampling, boot history and the loader are all refuted, and only a power-on (not reset) capture can name the live 8-byte block silicon holds and the emulator does not |
| [reference-images-are-not-reproducible-across-hosts](reference-images-are-not-reproducible-across-hosts.md) | carried | 2026-09-07 | scripts/emu/build-reference-image.sh + CI's `emu-c6` job | two builds on one host are byte-identical; between hosts they differ by ~4.7 KB (the rustc binary's own build), so every memory-class figure in a grade-3+ gate stays exact but every positional figure (stack high-water, instruction count) is a band with a digest, not a number; closing it wants one pinned build container |
