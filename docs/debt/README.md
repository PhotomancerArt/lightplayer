...
| --- | --- | --- | --- | --- |
| [heap-budget-record-churns-on-routine-changes](heap-budget-record-churns-on-routine-changes.md) | carried | 2026-09-06 | heap ratchet (scripts/heap-budget-*) | 37 of 67 commits to the record are re-baselines; the C6/S3 `stackTotal` "exact" figure moves with every byte of statics, and two re-baselining PRs always conflict on the one file |
| [nested-workspaces-miss-root-patches](nested-workspaces-miss-root-patches.md) | carried | 2026-07-30 | Cargo workspaces outside the root | a nested workspace (lp-xt/fixtures) silently builds product crates against unpatched crates.io forks; breaks CI only when the fork's behaviour is needed |
