...
| --- | --- | --- | --- | --- |
| [heap-budget-record-churns-on-routine-changes](heap-budget-record-churns-on-routine-changes.md) | retired | 2026-09-06 | heap ratchet (scripts/heap-budget-*) | 37 of 67 commits to the record are re-baselines; the C6/S3 `stackTotal` "exact" figure moves with every byte of statics, and two re-baselining PRs always conflict on the one file; retired 2026-09-23 (#798) — `stackTotal` is derived from the ELF and the record is one file per project and per chip |
| [process-wide-heap-counters-in-tests](process-wide-heap-counters-in-tests.md) | paying-down | 2026-08-28 | host heap-measurement tests (lpc-engine, lpvm-native) | a `#[global_allocator]` with process-wide atomics also counts the libtest main thread (which allocates right after spawning the test) and backend helper threads, so a loaded runner reds a retained-heap test with bytes that are not the product's; PR #827's three counters went per-thread 2026-09-25, five older ones remain |
| [actions-cache-budget](actions-cache-budget.md) | paying-down | 2026-07-28 | CI / GitHub Actions cache | one 10 GB LRU budget shared by every CI cache; on 2026-09-25 the eviction horizon was 13 min (seven registry copies re-saved per manifest edit + PR-scoped sccache writes), so emu-ref/stories caches missed; now one registry entry and main-only writes |
| [nested-workspaces-miss-root-patches](nested-workspaces-miss-root-patches.md) | retired | 2026-07-30 | Cargo workspaces outside the root | a nested workspace (lp-xt/fixtures) silently builds product crates against unpatched crates.io forks; breaks CI only when the fork's behaviour is needed; retired 2026-09-23 when `just lint-nested-patches` (in check-lint) made a missing or mismatched entry fail with the crate named |
| [emu-c6-air-delivers-every-other-frame](emu-c6-air-delivers-every-other-frame.md) | retired | 2026-09-08 | lp-emu-esp32c6 `machine.rs` delivery walk + `periph/wifi_stub.rs` RX cursors | a guest on the emulated air surfaced every OTHER frame written into its ring; retired 2026-09-09 — the delivery re-derived its write position from the ring base each frame and the blob recycles a consumed descriptor to the tail while moving that base one ISR later, so the modelled DMA now holds a cursor and steps it |
...
| [output-flush-refusal-is-log-only](output-flush-refusal-is-log-only.md) | carried | 2026-09-02 | lpc-engine output flush + lpa-server tick + lpc-wire project read | the engine names the wire it refused and `LpServer::advance_frame` drops it into a log line, so a project driving a wire its board lacks reads exactly like one that opened — no badge, no fault, no status, and no client-side test of board strictness |
...
| [two-green-prs-can-red-main](two-green-prs-can-red-main.md) | paying-down | 2026-08-02 | CI / merge policy | CI never builds the merge result, so two PRs touching opposite sides of an interface both pass and main breaks; cost is misattributed blame — since 2026-09-24 a red main push names the PRs merged since the last green main (`red-main-suspects` job, #800); retire once a real incident is diagnosed from it |
...
| [lps-probe-perf-test-load-sensitive](lps-probe-perf-test-load-sensitive.md) | retired | 2026-08-01 | lps-probe/tests | retired 2026-09-24: the wall-clock test is `#[ignore]`d and run by `just perf-probe`; the gate keeps an untimed compile+probe of the same shader |
...
| [web-serial-js-untestable](web-serial-js-untestable.md) | retired | 2026-07-10 | lpa-link/browser-serial | JS session/flash layer ships untested; bugs surface only on hardware — retired 2026-09-09 when emulator plan two put a `navigator.serial` polyfill under the shipped JS and a 15-test conformance suite (in CI) over it; Chromium's own USB stack stays hardware-only by design |
...
| [example-shaders-not-compile-gated](example-shaders-not-compile-gated.md) | retired | 2026-07-29 | examples GLSL + CI + lps-filetests | an example shader could compile on the host yet fail on 4 of 5 targets; retired 2026-09-24 (PR #812) when `just test-example-shaders` began compiling every catalog/ + projects/test/ shader on all `ALL_TARGETS` behind the `shader` gate |
...
| [wasm-cloud-check-not-in-just-check](wasm-cloud-check-not-in-just-check.md) | retired | 2026-08-07 | justfile local gate vs lpa-cloud-client wasm32 build | `check-wasm-cloud` (the fast gate for lpa-cloud-client's browser feature combo) was not in `just check`'s chain, so only `just studio-web-build` (slow) would catch a regression; retired 2026-09-24 when it joined the chain (warm ~1 s, cold ~47 s) |
...
| [regalloc-walk-superlinear-passes](regalloc-walk-superlinear-passes.md) | carried | 2026-06 | lpvm-native regalloc walk | per-loop liveness re-walks the whole subtree per nesting level and the edit list repairs order by insertion sort — time-only, noise at ~2K VInsts, real at dome-scale instruction counts |
| [failed-only-asset-edit-header-blindness](failed-only-asset-edit-header-blindness.md) | carried | 2026-08-01 | lpa-studio-web session·project control + lpa-studio-core project header actions | a rejected/oversize asset edit shows error tint in the header but no count and no Save/Revert (`persisted=0, failed>0`); with the Tree save row retired, the panel's per-entry revert is the only exit |
