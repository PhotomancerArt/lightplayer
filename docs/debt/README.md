...
| [s3-oom-reported-as-panic](s3-oom-reported-as-panic.md) | carried | 2026-08-02 | lp-fw/fw-esp32s3 | an allocation failure is staged as `CrashCause::Panic` with no heap stats, not `Oom` — the C6 and v3 classify it correctly, so anything keying on the cause silently misses a real OOM |
| [black-fallback-warning-floods-the-console](black-fallback-warning-floods-the-console.md) | carried | 2026-08-02 | lp-core/lpc-engine shader node | a quarantined shader logs its black fallback every frame — 90,020 lines in one bench run — saturating the serial link the host needs to recover the device |
