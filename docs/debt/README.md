...
| [output-channel-led-cap-silent-truncation](output-channel-led-cap-silent-truncation.md) | carried | 2026-05-18 | lp-fw/fw-esp32-common + fw-esp32c6 output | `MAX_LEDS = 256` per-channel cap is silent (no log, no error) and duplicated in two crates; a long strip renders truncated with no diagnostic |
