...
unsafe extern "C" fn ble_hs_rx_data(om: *const OsMbuf, arg: *const c_void) -> i32 {
    ...
    super::BT_STATE.with(|state| {
        ...
        data[0] = 0x02; // ACL
        // LP fork (third_party/esp-radio/README-LP.md): the controller can
        // hand over one ACL packet as a CHAIN of mbufs. Upstream copied only
        // the first mbuf's `om_len` bytes, so the packet reached the host
        // shorter than its own header says and bt-hci refused to parse it.
        let mut len = 1;
        let mut segments = 0;
        let mut m = om;
        while !m.is_null() {
            let seg = unsafe { core::slice::from_raw_parts((*m).om_data, (*m).om_len as usize) };
            let take = seg.len().min(data.len() - len);
            data[len..][..take].copy_from_slice(&seg[..take]);
            len += take;
            segments += 1;
            if take < seg.len() {
                warn!("[hci] ACL packet longer than {} B cut", data.len() - 1);
                break;
            }
            m = unsafe { (*m).next };
        }
        if segments > 1 {
            trace!("[hci] ACL packet of {} B joined from {} mbufs", len - 1, segments);
        }

        state.rx_queue.push_back(ReceivedPacket {
            data: Box::from(&data[..len]),
        });

        super::dump_packet_info(&data[..len]);
    });
