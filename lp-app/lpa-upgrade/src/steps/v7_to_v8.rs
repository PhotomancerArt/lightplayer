//! Format 7 → 8: shader GLSL entries are dimension-explicit.
//!
...
//! No older project could author `OneD` through a released build (the
//! declaration ships with this bump), so every pre-v8 GLSL asset defines
//! the 2D entry under the old name. (This step renumbered twice: v6 went
//! to the uid re-render, v7 to the float_mode pin — each merged first.)
//!
//! ## What this step does
//!
...
mod tests {
    ...
    fn a_glsl_asset_is_rewritten_alongside_the_manifest() {
        let mut files: ProjectFiles = [
            ("project.json", b"{\"format\": 7}".to_vec()),
            (
                "shader.glsl",
