//! Format 5 → 6: shader GLSL entries are dimension-explicit.
//!
...
//! No version-5 project could author `OneD` (the declaration did not exist
//! yet), so every pre-v6 GLSL asset defines the 2D entry under the old name.
//!
//! ## What this step does
//!
...
//!
//! `project.json`'s own `format` field is bumped `5` → `6` the same way
