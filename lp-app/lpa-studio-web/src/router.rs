...
//!   dispatch: to `Home` while the editor is open = lens detach
...
impl StudioRoute {
    ...
    pub(crate) fn parse(path: &str) -> Self {
        ...
        let (path, _fragment) = path.split_once('#').unwrap_or((path, ""));
...
mod tests {
    ...
    fn routes_round_trip_through_their_path() {
        for route in every_route() {
            ...
            assert!(!path.contains('#'), "{path:?} still carries a fragment");
