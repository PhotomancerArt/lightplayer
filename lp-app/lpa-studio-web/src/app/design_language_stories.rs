    ...
    rsx! {
        article { class: "tw:grid tw:max-w-[860px] tw:min-w-0 tw:gap-7 tw:p-1",
            ...
            Idiom {
                ...
                rule: "The iridescent ring answers hover, the flare answers press, the pinned ring plus a lifted shadow rides a drag — on CARDS and buttons. Dense rows go quiet instead: a plain white outline on hover, flashing spectrum on press (the preview of the selection ring a click lands). Pointer light is EXCLUSIVELY transient — nothing at rest may animate.",
                button { class: "{demo_chip} {ir_ring_class()} ux-press-flare tw:cursor-pointer", "hover / press me" }
                div { class: "tw:grid tw:w-44 tw:gap-1",
                    div { class: "{row_edge_class()} tw:cursor-default tw:rounded-sm tw:px-2.5 tw:py-1.5 tw:text-xs tw:text-soft-foreground",
                        "a dense row (hover / press me)"
                    }
                }
