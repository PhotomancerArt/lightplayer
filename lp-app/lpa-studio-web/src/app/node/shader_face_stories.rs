...
#[story(
    description = "The MODIFIER ROWS, MUTUALLY REFLECTIVE — the same shape (radial) under each modifier combination: plain, mirrored, flipped, mirrored+flipped. Each modifier is its own TWO-CARD single-select row in exactly the shape row's treatment (the amendment: 'each should get a row with two cards… exactly the same treatment as the show in 2d by'): mirror = [normal | mirrored], flip = [normal | flipped], selected card = the bool. Every face is a true what-if: the SHAPE tiles redraw with the card's current mirror/flip applied (watch the radial tile change across the four cards), and each modifier row's two cards draw the current shape and other modifier with its own bit off vs on. One chain-derived drawing function feeds every face; captions read `radial · mirrored · flipped`. Along-the-wire's forward/reversed is the same two-card row shape."
)]
