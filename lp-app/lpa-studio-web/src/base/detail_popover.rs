...
/// (no gap — sections carry their own padding), liquid-glass material
/// (`ux-glass-panel` owns background, blur, border, edge light, shadow,
/// and the spectrum ring — Aurora R3).
///
...
pub fn detail_popover_card_class() -> &'static str {
    "tw:grid tw:w-[min(320px,calc(100vw-24px))] tw:gap-0 tw:overflow-hidden tw:rounded-md tw:text-sm tw:text-muted-foreground ux-glass-panel"
}
