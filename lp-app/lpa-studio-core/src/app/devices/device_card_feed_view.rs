...
pub enum FeedLiveness {
    ...
    Offline,
    /// The editor lens holds the wire, so the feed is paused: the last
    /// frame, dimmed, with "editor has the wire".
    Lens,
}
...
pub fn device_card_feed_views(
    ...
    views
        ...
        .filter_map(|view| {
            ...
            device_card_feed_view(device, view, feed, effects, feeds.page_visible(), now)
