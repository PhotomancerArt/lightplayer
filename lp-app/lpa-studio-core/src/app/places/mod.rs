...
//!
//! The library is the source of truth; devices are places projects are
//! pushed to and pulled from. The trait here is deliberately small — it
//! establishes the seam (kind + capacity) — and the ops live on the
//! concrete types until real callers shape the abstraction. The vestigial
//! `RuntimePlace` (zero callers, and a `SimRuntime` kind the model no
//! longer has) went with the sim's own runtime, PD9.
//!
